...
bool parse_invite_args(const std::string &rest, std::string &target, std::string &chname, const Client &cl, int fd, std::map<int, Client> &clients)
{
	// INVITE <nick> <#chan>
...
bool lookup_target_fd(const std::string &target, int &out_fd, const Client &cl, int fd, std::map<int, Client> &clients)
{
	std::map<std::string,int>::iterator it =
		g_state.nick2fd.find(lower_str(target));
	if (it == g_state.nick2fd.end()) {
		send_numeric(clients, fd, NO_SUCH_NICK, cl.nick, target, "No such nick");
		return false;
	}
	out_fd = it->second;
	return true;
...
bool ensure_invite_privs(Channel &ch, const std::string &chname, const Client &cl, int fd,  std::map<int, Client> &clients)
{
...
inline void reply_inviting(std::map<int, Client> &clients, int fd, const Client &cl, const std::string &target, const std::string &chname)
{
	send_numeric(clients, fd, INVITING, cl.nick, target + " " + chname, "");
}

inline void notify_invited(std::map<int, Client> &clients, int rfd, const Client &cl, const std::string &target, const std::string &chname)
{
	enqueue_line(clients, rfd, ":" + cl.nick + " INVITE " + target + " :" + chname);
}
