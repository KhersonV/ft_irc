...
bool parse_privmsg(const std::string &rest, std::string &target, std::string &text, const Client &cl, int fd, std::map<int, Client> &clients)
{
	if (rest.empty()) {
		send_numeric(clients, fd, NO_RECIPIENT, cl.nick, "PRIVMSG",
...
bool send_privmsg_to_channel(std::map<int, Client> &clients, int fd, const Client &cl, const std::string &target_chan, const std::string &text)
{
	Channel *ch = 0;
	...
	const std::string line = ":" + cl.nick + " PRIVMSG " + target_chan + " :" + text;
...
inline void send_privmsg_to_user(std::map<int, Client> &clients, int rfd, const Client &cl, const std::string &target_nick, const std::string &text)
{
	const std::string from = cl.nick.empty() ? "*" : cl.nick;
	const std::string line = ":" + from + " PRIVMSG " + target_nick + " :" + text;
...
bool handle_PRIVMSG(int fd, Client &cl, std::map<int, Client> &clients, const std::string &rest)
{
	...
	if (!parse_privmsg(rest, target, text, cl, fd, clients))
		return false;

	if (!target.empty() && target[0] == '#') {
		send_privmsg_to_channel(clients, fd, cl, target, text);
		return false;
	}
	...
	send_privmsg_to_user(clients, rfd, cl, target, text);
	return false;
}
